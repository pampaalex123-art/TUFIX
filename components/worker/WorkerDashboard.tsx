  ...
  return (
    <div className="container mx-auto max-w-5xl space-y-8">
      <div className="bg-slate-50 border border-slate-200 p-6 rounded-xl shadow-sm flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-black">{t('welcome back', { name: worker.name })}</h1>
          <p className="text-black mt-1">{t('worker dashboard subtitle')}</p>
        </div>
      </div>
                ...
                <div key={request.id} className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 hover:border-purple-200 transition-colors">
                  <div className="flex flex-col md:flex-row justify-between">
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        <div className="flex items-center space-x-3 flex-wrap">
                          ...
                          {request.status === 'pending' && <span className="text-xs bg-yellow-100 text-yellow-800 font-medium px-2.5 py-0.5 rounded-full">New</span>}
                          {request.status === 'accepted' && <span className="text-xs bg-green-100 text-green-800 font-medium px-2.5 py-0.5 rounded-full">Accepted</span>}
                          {request.status === 'completed' && <span className="text-xs bg-blue-100 text-blue-800 font-medium px-2.5 py-0.5 rounded-full">Completed</span>}
                           {isDisputeActive && <span className="text-xs bg-red-100 text-red-800 font-medium px-2.5 py-0.5 rounded-full">Disputed</span>}
                        </div>
                        <p className="text-sm text-black mt-1">{request.user.location} &bull; Requested on {formatDate(request.date)}</p>
                    ...
                    <div className="mt-4 md:mt-0 flex flex-col items-stretch md:items-end space-y-2 md:pl-4">
                        <button onClick={() => onSelectJob(request)} className="w-full text-center md:w-auto bg-purple-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-purple-700">{t('view details')}</button>
                         {request.status === 'completed' && invoice && invoice.status === 'held' && (
                            isDisputeActive ? 
                            <button onClick={() => onViewDispute(request.disputeId!)} className="text-sm font-semibold text-yellow-500 hover:underline text-right">{t('view dispute')}</button>
                            : <button onClick={() => onRaiseDispute(request)} className="text-sm font-semibold text-red-500 hover:underline text-right">{t('raise dispute')}</button>
                        )}
                    </div>
                  </div>
                </div>
