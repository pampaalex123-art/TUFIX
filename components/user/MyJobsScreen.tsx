...
const MyJobsScreen: React.FC<MyJobsScreenProps> = ({ jobRequests, invoices, workers = [], userCountry, onLeaveReview, onCancelJob, onBack, onPayInvoice, onConfirmAndReleasePayment, onRaiseDispute, onViewDispute, onUpdateJobLocation, t }) => {
  ...
  const applyFilters = (jobs: JobRequest[]) => jobs.filter(j => {
    const w = workers.find(w => w.id === j.workerId);
    if (filterName.trim() && !(w?.name || '').toLowerCase().includes(filterName.toLowerCase())) return false;
  ...
  const sortedJobs = showPastJobs ? filteredPast.slice(0, pastJobsCount) : filteredActive;
        ...
        {showFilter && (
          <div className="mb-4 p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Nombre del trabajador</label>
                <input
              ...
              <div key={job.id} className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm grid grid-cols-1 md:grid-cols-3 gap-4 hover:border-purple-200 transition-colors">
                <div className="md:col-span-2 p-2">
                  ...
                  <div className="mt-6 flex flex-wrap gap-2 items-center">
                    ...
                    {job.status === 'worker_completed' && invoice && invoice.status === 'held' && <button onClick={() => handleConfirmCompletion(job)} className="bg-green-600 text-white font-bold py-2 px-4 rounded-lg">{t('confirm completion')}</button>}
