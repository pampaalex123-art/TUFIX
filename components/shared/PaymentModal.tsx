...
// Inline MercadoPago logo — always renders, no external URL
...
const PaymentModal: React.FC<PaymentModalProps> = ({ invoice, job, onClose, onConfirm, onUpdateLocation, t }) => {
    ...
    { 
      ...
      icon: (
        <img src="https://upload.wikimedia.org/wikipedia/commons/5/5e/Visa_Inc._logo.svg" alt="Visa"
          className="w-10 h-auto max-h-full object-contain" referrerPolicy="no-referrer"
  ...
  const handleMercadoPagoPayment = async () => {
    try {
      const response = await fetch('/api/mercadopago/create-preference', {
        ...
        body: JSON.stringify({ 
          items: [{ title: `Pago de Servicio TUFIX - #${invoice.id.slice(-6)}`, unit_price: invoice.total, quantity: 1, currency_id: invoice.currency || 'ARS' }],
  ...
  const handleOtherPayment = (method: PaymentMethod) => {
    ...
    else { alert(`El método ${method} estará disponible próximamente.`); setLoading(false); }
  };
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (selectedMethod === 'mercadopago') { await handleMercadoPagoPayment(); } 
      else { handleOtherPayment(selectedMethod); }
    } catch (error) {
      ...
      <div className="bg-white w-full max-w-md rounded-t-[24px] sm:rounded-[24px] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Total: <span className="font-bold text-purple-600">{formatCurrency(invoice.total, invoice.currency)}</span></p>
          </div>
          <button onClick={onClose} className="p-2 bg-gray-50 rounded-full text-gray-400 hover:text-gray-600 transition-colors"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="p-6 bg-white border-t border-gray-100 space-y-3">
          <button onClick={handleContinue} disabled={loading}
            ...
            {loading ? <div className="w-6 h-6 border-3 border-white border-t-transparent rounded-full animate-spin" /> : <><span className="text-base">Pagar Factura</span><ChevronRight className="w-5 h-5" /></>}
          </button>
          <button onClick={onClose} className="w-full py-2 px-6 text-gray-400 text-sm font-medium hover:text-gray-600 transition-colors">Volver</button>
        </div>
      </div>
    </div>
  );
